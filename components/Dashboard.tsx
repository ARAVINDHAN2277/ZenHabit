...
export const Dashboard: React.FC<DashboardProps> = ({ habits, userName = "Zen User" }) => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(22);
      pdf.text("ZenHabit 2026", 15, 20);
      pdf.setFontSize(10);
      pdf.text("OFFICIAL PERFORMANCE ANALYSIS REPORT", 15, 30);

      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(12);
      pdf.text(`ANALYST: ${userName}`, pdfWidth - 15, 25, { align: 'right' });
      
      pdf.addImage(dataUrl, 'PNG', 0, 45, pdfWidth, pdfHeight);
      
      pdf.save(`ZenHabit_Insights_2026_${userName.replace(/\s+/g, '_')}.pdf`);
    } catch (err) {
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
         ...
         <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="flex bg-slate-100 p-1.5 rounded-2xl flex-1 sm:flex-none">
                <button 
                  ...
                  className={`flex-1 sm:px-8 py-2.5 rounded-xl text-xs font-bold transition-all ${viewMode === 'overview' ? 'bg-white text-indigo-600 shadow-lg' : 'text-slate-500'}`}
                >Overview</button>
                <button 
                  onClick={() => setViewMode('habits')} 
                  className={`flex-1 sm:px-8 py-2.5 rounded-xl text-xs font-bold transition-all ${viewMode === 'habits' ? 'bg-white text-indigo-600 shadow-lg' : 'text-slate-500'}`}
                >Leaderboard</button>
            </div>
            <button 
              onClick={downloadPDF}
              disabled={isExporting}
              className="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-slate-900 text-white text-xs font-black shadow-lg hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-50"
            >
              <i className={`fa-solid ${isExporting ? 'fa-spinner fa-spin' : 'fa-file-pdf text-rose-400'}`}></i>
      ...
      <div ref={dashboardRef} className="space-y-8">
        {/* Personalized Welcome (Only visible during capture/on dashboard) */}
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 p-8 sm:p-10 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h1 className="text-3xl sm:text-4xl font-black tracking-tight mb-2">2026 Growth Summary</h1>
                  <p className="text-indigo-100 font-medium text-sm sm:text-base">Comprehensive performance audit for <span className="text-white font-black underline underline-offset-4">{userName}</span></p>
                </div>
                <div className="flex items-center gap-3 bg-white/10 px-6 py-4 rounded-3xl backdrop-blur-md border border-white/20">
                   <div className="text-right">
                      <p className="text-[10px] font-black text-indigo-200 uppercase">Mastery Index</p>
                      <p className="text-2xl font-black">{analytics.avgRate.toFixed(1)}%</p>
                   </div>
                   <div className="w-12 h-12 bg-white text-indigo-600 rounded-2xl flex items-center justify-center text-xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {[
            ...
            { label: 'Peak Streak', val: `${analytics.maxStreak} Days`, icon: 'fa-fire-flame-curved', color: 'text-orange-600', bg: 'bg-orange-50' },
            { label: 'Top Pillar', val: analytics.bestCat, icon: 'fa-crown', color: 'text-emerald-600', bg: 'bg-emerald-50' },
            { label: 'Zen Momentum', val: analytics.totalZen, icon: 'fa-leaf', color: 'text-purple-600', bg: 'bg-purple-50' },
          ].map((item, i) => (
            <div key={i} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
              <div className={`w-12 h-12 ${item.bg} ${item.color} rounded-2xl flex items-center justify-center text-lg mb-5`}>
                ...
                <div className="h-[380px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={yearlyCategoryTrends}>
                      ...
                      {CATEGORIES.map(cat => (
                        <Line key={cat} type="monotone" dataKey={cat} stroke={COLORS[cat]} strokeWidth={4} dot={false} activeDot={{ r: 8 }} />
                      ))}
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                  ...
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={monthlyVolume}>
                        ...
                        <Tooltip />
                        {CATEGORIES.map(cat => (
                          <Area key={cat} type="monotone" dataKey={cat} stackId="1" stroke={COLORS[cat]} fill={COLORS[cat]} fillOpacity={0.5} />
                        ))}
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12">
                ...
                <div className="flex items-center gap-4 bg-slate-800/40 p-4 rounded-[20px] border border-slate-700/50">
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="w-5 h-5 rounded-md bg-indigo-700"></div>
                         ...
                         <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 sm:gap-2.5">
                            {Array.from({ length: MONTH_DAYS[mIdx] }).map((_, dIdx) => {
                               ...
                               return (
                                 <div 
                                   key={dIdx} 
                                   className="w-4 h-4 sm:w-6 sm:h-6 rounded-lg transition-all duration-500"
                                   style={{ 
